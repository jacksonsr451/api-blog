...
def test_create_post_entity_without_title_raises_error():
...
def test_publish_post_without_description_or_body_raises_error():
    ...
    with pytest.raises(ValueError, match="Post must have a description and body before being published."):
        post.publish()
...
def test_repr_method():
    ...
    post.create_a_new_post(description="Test Description", body="Test Body")
    assert repr(post) == f"<PostEntity(id={post.id}, title=Test Repr, status=draft)>"
    post.publish()
    assert repr(post) == f"<PostEntity(id={post.id}, title=Test Repr, status=published)>"
